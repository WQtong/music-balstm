...
# This part is used to get "position", "pitchclass", "previous vicinity", "previous context" and "beat" features.
...
# This part combines all the features in previous part to form a input with size (batch, time, notes, 80)
...
# This two functions are used to get train/test batch in the training/testing processes

def getPieceSegment(pieces, batch_len):
...
def getPieceBatch(pieces, batch_width, batch_len):
    i,o = zip(*[getPieceSegment(pieces, batch_len) for _ in range(batch_width)])
    return np.array(i), np.array(o)

# This two functions are used to store and load the dictionary of midi matrices so that we can transfer it
# into midi file on our local machine that runs Python 2
